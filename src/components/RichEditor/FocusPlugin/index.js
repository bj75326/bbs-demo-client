...
import defaultTheme from './index.less';
...
export default (config = {}) => {
  ...
  return {
    ...
    handleKeyCommand: (
      ...
      if (
        ...
        if (newEditorState !== editorState) {
          console.log('handleKeyCommand handled!!!!!!!');
          setEditorState(newEditorState);
          return 'handled';
        }
      }
      console.log('handleKeyCommand not-handled!!!!');
      return 'not-handled';
    },

    onChange: editorState => {
      console.log('focusPlugin onchange run');
      ...
      if (!contentState.equals(lastContentState)) {
        console.log('quite focusPlugin onChange #1');
      ...
      if (lastSelection && selection.equals(lastSelection)) {
        console.log('quite focusPlugin onChange #2');
      ...
      if (lastSelection) {
        ...
        if (lastBlockMapKeys.some(key => focusableBlockKeys.includes(key))) {
          console.log('quite focusPlugin onChange #3');
    ...
    keyBindingFn(evt, { getEditorState, setEditorState }) {
      console.log('keyBindingFn run!!!!!!!!!!!!', evt.keyCode);
      const editorState = getEditorState();
      // TODO match by entitiy instead of block type
      if (focusableBlockIsSelected(editorState, blockKeyStore)) {
        console.log('focusableBlockIsSelected!!!!!!!!!!');
        // arrow left
        if (evt.keyCode === 37) {
    ...
    blockRendererFn: (contentBlock, { getEditorState, setEditorState }) => {
      ...
      //get 'noFocus' from entity
      let noFocus = false;
      if(contentBlock.getEntityAt(0) !== null){
        noFocus = editorState.getCurrentContent().getEntity(contentBlock.getEntityAt(0)).get('noFocus');
      }
      ...
      return {
        props: {
          ...
          //sometimes, we need to involve other logic in click handler
          setSelectionToBlock,
        },
      };
    },
